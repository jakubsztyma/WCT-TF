...
def pad_reflect(x, padding=1):
    return tf.pad(
        tensor=x, paddings=[[0, 0], [padding, padding], [padding, padding], [0, 0]],
        mode='CONSTANT')


def Conv2DReflect(*args, **kwargs):
    '''Wrap Keras Conv2D with reflect padding'''
    return Lambda(lambda x: Conv2D(*args, kernel_size=3, padding='valid', **kwargs)(pad_reflect(x)))


def Conv2DRelu(*args, **kwargs):
    return Conv2DReflect(*args, activation='relu', **kwargs)
...
def wct_tf(content, style, alpha, eps=1e-8):
    ...
    # content_t = tf.transpose(a=tf.squeeze(content), perm=(2, 0, 1))
    # style_t = tf.transpose(a=tf.squeeze(style), perm=(2, 0, 1))
    content_t = tf.transpose(a=content[0], perm=(2, 0, 1))
    style_t = tf.transpose(a=style[0], perm=(2, 0, 1))

    Cc, Hc, Wc = tf.unstack(tf.shape(input=content_t))
    Cs, Hs, Ws = tf.unstack(tf.shape(input=style_t))
    ...
    # Content covariance
    mc = tf.reduce_mean(input_tensor=content_flat, axis=1, keepdims=True)
    fc = content_flat - mc
    fcfc = tf.matmul(fc, fc, transpose_b=True) / (tf.cast(Hc * Wc, tf.float32) - 1.) + tf.eye(Cc) * eps

    # Style covariance
    ms = tf.reduce_mean(input_tensor=style_flat, axis=1, keepdims=True)
    fs = style_flat - ms
    ...
    k_c = tf.reduce_sum(input_tensor=tf.cast(tf.greater(Sc, 1e-5), tf.int32))
    k_s = tf.reduce_sum(input_tensor=tf.cast(tf.greater(Ss, 1e-5), tf.int32))

    # Whiten content feature
    Dc = tf.linalg.tensor_diag(tf.pow(Sc[:k_c], -0.5))
    fc_hat = tf.matmul(tf.matmul(tf.matmul(Uc[:, :k_c], Dc), Uc[:, :k_c], transpose_b=True), fc)

    # Color content with style
    Ds = tf.linalg.tensor_diag(tf.pow(Ss[:k_s], 0.5))
    ...
    # CxHxW -> 1xHxWxC
    blended = tf.expand_dims(tf.transpose(a=blended, perm=(1, 2, 0)), 0)
...
def wct_style_swap(content, style, alpha, patch_size=2, stride=2, eps=1e-8):
    ...
    whiten_content = tf.expand_dims(tf.transpose(a=tf.reshape(fc_hat, [Cc, Hc, Wc]), perm=[1, 2, 0]), 0)

    ### Whiten style before swapping
    Ds = tf.linalg.tensor_diag(tf.pow(Ss[:k_s], -0.5))
    whiten_style = tf.matmul(tf.matmul(tf.matmul(Us[:, :k_s], Ds), Us[:, :k_s], transpose_b=True), fs)
    # Reshape before passing to style swap, CxH*W -> 1xHxWxC
    whiten_style = tf.expand_dims(tf.transpose(a=tf.reshape(whiten_style, [Cs, Hs, Ws]), perm=[1, 2, 0]), 0)

    ### Style swap whitened encodings
    ss_feature = style_swap(whiten_content, whiten_style, patch_size, stride)
    # HxWxC -> CxH*W
    ss_feature = tf.transpose(a=tf.reshape(ss_feature, [Hc * Wc, Cc]), perm=[1, 0])

    ### Color style-swapped encoding with style 
    Ds_sq = tf.linalg.tensor_diag(tf.pow(Ss[:k_s], 0.5))
...
def style_swap(content, style, patch_size, stride):
    ...
    '''
    nC = tf.shape(input=style)[-1]  # Num channels of input content feature and style-swapped output

    ### Extract patches from style image that will be used for conv/deconv layers
    style_patches = tf.image.extract_patches(style, [1, patch_size, patch_size, 1], [1, stride, stride, 1],
                                             [1, 1, 1, 1], 'VALID')
    before_reshape = tf.shape(input=style_patches)  # NxRowsxColsxPatch_size*Patch_size*nC
    style_patches = tf.reshape(style_patches, [before_reshape[1] * before_reshape[2], patch_size, patch_size, nC])
    style_patches = tf.transpose(a=style_patches, perm=[1, 2, 3, 0])  # Patch_sizexPatch_sizexIn_CxOut_c
    ...
    ss_argmax = tf.argmax(input=ss_enc, axis=3)
    encC = tf.shape(input=ss_enc)[-1]  # Num channels in intermediate conv output, same as # of patches
    ...
    deconv_out_H = utils.deconv_output_length(tf.shape(input=ss_oh)[1], patch_size, 'valid', stride)
    deconv_out_W = utils.deconv_output_length(tf.shape(input=ss_oh)[2], patch_size, 'valid', stride)
    ...
    ss_oh_sum = tf.reduce_sum(input_tensor=ss_oh, axis=3, keepdims=True)
...
def adain(content_features, style_features, alpha, epsilon=1e-5):
    ...
    '''
    style_mean, style_variance = tf.nn.moments(x=style_features, axes=[1, 2], keepdims=True)
    content_mean, content_variance = tf.nn.moments(x=content_features, axes=[1, 2], keepdims=True)
...
def torch_decay(learning_rate, global_step, decay_rate, name=None):
    ...
    with tf.compat.v1.name_scope(name, "ExponentialDecay", [learning_rate, global_step, decay_rate]) as name:
        learning_rate = tf.convert_to_tensor(value=learning_rate, name="learning_rate")
