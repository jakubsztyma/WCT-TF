...
2. Train decoders (described below)
...
4. Run stylization with `stylize.py`. For instance, to run a multi-level stylization pipeline that goes from relu5_1 -> relu4_1 -> relu3_1 -> relu2_1 -> relu1_1:
...
* `--style-paths`  One or two style images
...
* `--beta`  [0,1] the dominanca of style 1 over style b
